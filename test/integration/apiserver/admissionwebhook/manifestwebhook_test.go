...
`), os.FileMode(0755)); err != nil {
	...
	generic.ServerContext, cancel = context.WithCancel(context.TODO())
	defer cancel()
...
`), os.FileMode(0755)); err != nil {
	...
	expectGaugeValue(t, grabber, "apiserver_admission_gke_webhook_manifest_error", 0)

	// change config to wrong config
	writeManifestFile(t, manifestFile, false, validatingWebhook, "v1beta1")
	time.Sleep(time.Second * 2)
	expectGaugeValue(t, grabber, "apiserver_admission_gke_webhook_manifest_error", 1)
	upCh := recorder.Reset()
...
func expectGaugeValue(t *testing.T, grabber *e2emetrics.Grabber, name string, want float64) {
	received, err := grabber.GrabFromAPIServer()
	if err != nil {
		t.Fatalf("failed to grab metrics: %v", err)
	}

	val := received[name][0].Value

...
func (w *ManifestWatcher) Init(webhookType WebhookType, ctx context.Context) error {
	if w.manifestFile == "" {
		return nil
	}
	var err error
	...
	if ctx == nil {
		return fmt.Errorf("cannot initilize with nil context")
	}
	w.ctx = ctx
...
func (w *ManifestWatcher) watchForChanges(watcher *fsnotify.Watcher) {
	...
	t := time.NewTimer(pollDuration)
	for {
		t.Reset(pollDuration)
		select {
...
func (w *ManifestWatcher) loadConfig() error {
	...
	accessors, foundNew, sha, err := extractWebhookAccessors(w.manifestFile, w.prevSha, w.webhookType, w.defaulter, w.validator)
	if foundNew {
		w.lastErr = err
		if err == nil {
...
// manifest file and any err encountered while parsing the file. It is to be noted
// that the returned sha would be non-nil even in the case of an non-nil error.
func extractWebhookAccessors(manifestFile string, prevSha string,
	webhookType WebhookType, defaulter WebhookDefaulter,
	validator WebhookValidator) ([]webhook.WebhookAccessor, bool, string, error) {
	if manifestFile == "" {
		return []webhook.WebhookAccessor{}, false, prevSha, nil
	}

	var err error
	data, err := ioutil.ReadFile(manifestFile)
	if err != nil {
		return []webhook.WebhookAccessor{}, false, prevSha, err
	}

	currentSha := fmt.Sprintf("%x", sha256.Sum256(data))
	if currentSha == prevSha {
	...
	mutatingKind := v1.SchemeGroupVersion.WithKind("MutatingWebhookConfiguration")
	validatingKind := v1.SchemeGroupVersion.WithKind("ValidatingWebhookConfiguration")
	var accessors []webhook.WebhookAccessor
	for _, item := range items {
		obj := item.Object
		switch kind := obj.GetObjectKind().GroupVersionKind(); kind {
		case validatingKind:
			if webhookType != ValidatingWebhook {
				continue
			}
			config := (obj).(*v1.ValidatingWebhookConfiguration)
			a, err := extractValidatingAccessors(config, defaulter, validator)
			if err != nil {
				return []webhook.WebhookAccessor{}, true, currentSha, err
			}
		...
		case mutatingKind:
			...
			a, err := extractMutatingAccessors(config, defaulter, validator)
			if err != nil {
				return []webhook.WebhookAccessor{}, true, currentSha, err
			}
			accessors = append(accessors, a...)
...
func extractMutatingAccessors(m *v1.MutatingWebhookConfiguration, defaulter WebhookDefaulter, validator WebhookValidator) ([]webhook.WebhookAccessor, error) {
	var accessors []webhook.WebhookAccessor
	if defaulter != nil {
...
func extractValidatingAccessors(v *v1.ValidatingWebhookConfiguration, defaulter WebhookDefaulter, validator WebhookValidator) ([]webhook.WebhookAccessor, error) {
	var accessors []webhook.WebhookAccessor
	if defaulter != nil {

...
type ManifestWebhookWrapper interface {
	// Initialize initializes the the manifest wrapper and returns any error
	// encountered. ctx is used to shutdown the wrappers gracefully when
	// ctx.Done() is called.
	Initialize(ctx context.Context, webhookType WebhookType) error
